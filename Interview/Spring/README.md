...
#### REQUIRED 
기본 옵션, 부모 트랜잭션이 존재한다면 부모 트랜잭션에 합류한다.   
그렇지 않다면 새로운 트랜잭션을 만든다. 중간에 자식/부모에서 Rollback이 발생하면 자식과 부모 모두 Rollback 한다.    
...
#### REQUIRED_NEW 
무조건 새로운 트랜잭션을 만든다.   
Nested(중첩)한 방식으로 메소드 호출이 이루어지더라도 Rollback은 각각 이루어진다.    
...
진행 중인 트랜잭션이 존재하면 Exception이 발생한다.   
