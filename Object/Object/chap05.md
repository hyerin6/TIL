...
가장 먼저 생각해볼 수 있는 해결 방법은 두 타입을 **두 개의 클래스로 분리**하는 것이다.  
그러나 두 개의 클래스로 분리하면 Movie의 인스턴스는 **두 개의 서로 다른 클래스의 인스턴스 모두와 협력**할 수 있어야 한다.      
이 문제의 해결 방법은 Movie 클래스 안에 SequenceCondition의 목록과 PeriodCondition의 목록을 따로 유지하는 것이다.     
하지만 Movie가 두 클래스 양쪽 모두에게 결합되어 **전체적인 결합도가 높아졌다.**   
두 번째 문제는 수정 후에 새로운 할인 조건을 추가하기 더 어려워졌다는 것이다.     
새로운 할인 조건 클래스를 담기 위한 리스트 변수를 Movie 클래스에 또 추가해야 한다.       

<br />

### 다형성을 통해 분리하기   
Movie의 입장에서 SequenceCondition와 PeriodCondition는 아무 차이도 없다.   
할인 여부를 판단한다는 동일한 책임을 수행하고 있을 뿐이다.    

역할의 개념이 다시 등장한다.   
Movie의 입장에서 동일한 책임을 수행하는 두 클래스는 동일한 역할을 수행한다는 것을 의미한다.   
역할은 협력 안에서 대체 가능성을 의미하기 때문에 역할에 대해서만 결합되도록 의존성을 제한할 수 있다.   

[개선된 전체 코드 보러가기](https://github.com/eternity-oop/object/tree/master/chapter05/src/main/java/org/eternity/movie/step04)    

DiscountCondition의 경우에서 객체의 암시적인 타입에 따라 행동을 분기해야 한다면 암시적인 타입을 명시적인 클래스로 정의하고   
행동을 나눔으로써 응집도 문제를 해결할 수 있다. 즉 객체의 타입에 따라 변하는 행동이 있다면   
타입을 분리하고 변화하는 행동을 각 타입의 책임으로 할당하라는 것이다.   
이를 POLYMORPHISM(다형성) 패턴이라고 부른다.   


```
객체의 타입에 따라 변하는 로직이 있을 때 로직을 담당할 책임을 어떻게 할당해야 할까?
타입을 명시적으로 정의하고 각 타입에 다형적으로 행동하는 책임을 할당하라.

if ~ else 또는 switch ~ case 등의 조건 논리를 사용해서 설계한다면 
새로운 변화가 일어났을 때 조건 논리를 수정해야 한다. 
이는 프로그램 수정을 어렵고 변경에 취약하게 만든다. 

POLYMORPHISM(다형성) 패턴은 객체의 타입을 검사해서 타입에 따라 수행하는 조건적인 논리를 사용하지 말라고 경고한다.   
...
### 변경으로부터 보호
SequenceCondition와 PeriodCondition은 서로 다른 이유로 변경된다.     
두 개의 서로 다른 변경이 두개의 서로 다른 클래스 안으로 캡슐화된다.    
...
### 변경과 유연성 
설계를 주도하는 것은 변경이다. 변경에 대비하는 두 가지 방법이 있다.   

* 코드를 이해하고 수정하기 쉽도록 최대한 단순하게 설계하는 것   
* 코드를 수정하지 않고 변경을 수용할 수 있도록 코드를 유연하게 만드는 것   

<br />


[개선된 전체 코드 보러가기](https://github.com/eternity-oop/object/tree/master/chapter05/src/main/java/org/eternity/movie/step05)
