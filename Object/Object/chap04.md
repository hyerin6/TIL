...
객체지향이 자신의 상태를 스스로 관리하는 자율적인 객체를 지향하는 것이라고 한다면 위 코드는 객체지향의 취지에 맞는 것처럼 보일 것이다.  

그런데 `isDiscountable` 메서드에 이상한 점이 있다.     

DiscountCondition에 속성으로 포함돼 있는 DayOfWeek 타입의 요일 정보와 LocalTime 타입의 시간 정보를 파라미터로 받는 것을 알 수 있다.     
이는 객체 내부에 DayOfWeek 타입의 요일, LocalTime 타입의 시간 정보가 인스턴스 변수로 포함돼 있다는 사실을   
인터페이스를 통해 외부에 노출하고 있는 것이다.   

만약 DiscountCondition의 속성을 변경해야 한다면?   
두 `isDiscountable` 메서드의 파라미터, 해당 메서드 사용 클라이언트를 전부 함께 수정해야 한다.   
내부 구현의 변경이 외부로 펴져나가는 파급 효과는 캡슐화가 부족하다는 명백한 증거다.     
자기 자신을 스스로 처리하고 있지만 여전히 내부의 구현을 캡슐화하는 데 실패했다.   

<br />

### 높은 결합도   
캡슐화 위반으로 인해 DiscountCondition의 내부 구현이 외부로 노출됐기 때문에    
Movie와 DiscountCondition 사이의 결합도는 높을 수밖에 없다.    

* DiscountCondition의 기간 할인 조건 명칭이 변경되면 Movie를 수정해야 한다.   
* DiscountCondition의 종류가 추가, 삭제되면 if~else 구문을 수정해야 한다.   
* 각 DiscountCondition의 만족 여부를 판단하는 데이터에 변경이 발생하면 
  Movie의 `isDiscountable` 메서드 파라미터를 변경, Movie의 메서드에 의존하는 Screening에 대한 변경도 초래한다.   


위 모든 문제의 원인은 캡슐화 원칙을 지키지 않았기 때문이다.   

<br />

### 낮은 응집도   
결과적으로 할인 조건의 종류를 변경하기 위해서는 DiscountCondition, Movie, Screening을 함께 수정해야 한다.   
응집도가 낮은 이유는 캡슐화를 위반했디 때문이다.   
DiscountConditio, Movie의 내부 구현이 인터페이스에 그대로 노출되고 있고   
...
객체지향 애플리케이션을 구현할 것이다. ➡️ 협력하는 객체들의 공동체를 구축할 것이다.    

객체가 내부에 어떤 상태를 가지고 그 상태를 관리하는가는 부가적인 문제다.   
중요한 것은 객체가 다른 객체와 협력하는 방법이다.    

실행 문맥에 대한 고민 없이 데이터의 세부 정보만 먼저 결정하면 객체의 구현이 이미 결정된 상태에서     
다른 객체와의 협력 방법을 고민하기 때문에 이미 구현된 객체의 인터페이스를 억지로 끼워맞출 수밖에 없다.     
