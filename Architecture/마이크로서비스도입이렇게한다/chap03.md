...
2. 새로운 마이크로서비스에서 기능 구현     
3. 모놀리스에 대한 호출을 신규 마이크로서비스에 대한 호출로 전환하게끔 라우팅     
...
#### 여전히 모놀리스를 사용할 필요가 있는 기능의 추출 
![img_5.png](../img/img_5.png)

사용자 통지 기능을 재구현하는 대신, (모놀리스 자체에 대한 변경이 명시적으로 필요할)     
사용자 통지 기능을 모놀리스에서 외부에 공개하는 방법으로 새로운 마이크로서비스에 노출시킨다.      

그러나 교살자가 작동하려면 인바운드 호출을 이동하려는 대상 자산에 대해 관심있는 기능으로 명확하게 매핑할 수 있어야 한다.     

```text
예) 우리는 고객에게 사용자 통지를 보내는 기능을 새로운 서비스로 이동하고 싶다.    
그러나 기존 모놀리스에 대한 인바운드 호출의 결과로 통지가 발생한다. => 시스템 외부에서 일어나는 호출을 명확하게 리다이렉션할 수 없다.   
```

<br />  

#### 이동할 기능이 기존 시스템 내부에 더 깊이 들어가 있으면 교살자 무화과 패턴이 제대로 작동하지 않는다.   

![img.png](../img/img_8.png)


기존 시스템에 대한 인바운드 호출의 성격도 고려해야 할 것이다.   

> 이런 제약에도 불구하고 교살자 무화과 애플리케이션은 유용한 마이그레이션 기법이다.   
> 가볍게 수행하는 능력과 점진적이인 변화를 다루는 손쉬운 접근 방법을 고려해보면   
> 시스템 마이그레이션 방법을 찾으려 할 때 최선의 선택지는 교살자 무화과 패턴이다.  
...
### 2-4) 사례: HTTP 리버스 프록시  
HTTP의 기능 중 호출하는 시스템에 투명하게 보이는 방법으로 호출을 가로채고 리디렉션하기 매우 쉬운 기능이 있다.    
이는 HTTP 인터페이스를 제공하는 기존 모놀리스를 교살자 무화과 패턴을 사용해 마이그레이션하기가 적합하다는 사실을 의미한다.   
